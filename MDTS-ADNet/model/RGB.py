...
class RGB_Encoder(nn.Module):
    def __init__(self, t_length=2, n_channel=3, latent_dim=128):
        ...
        self.interaction_conv1 = nn.Conv2d(64, 64, kernel_size=1)
        self.interaction_conv2 = nn.Conv2d(128, 128, kernel_size=1)
        self.interaction_conv3 = nn.Conv2d(256, 256, kernel_size=1)
        self.interaction_conv4 = nn.Conv2d(512, 512, kernel_size=1)
    ...
    def forward(self, x, flow_features=None):
        ...
        if flow_features is not None:
            tensorConv1 = self.interaction_conv1(tensorConv1 + flow_features[0])
        tensorPool1 = self.modulePool1(tensorConv1)

        tensorConv2 = self.moduleConv2(tensorPool1)
        if flow_features is not None:
            tensorConv2 = self.interaction_conv2(tensorConv2 + flow_features[1])
        tensorPool2 = self.modulePool2(tensorConv2)

        tensorConv3 = self.moduleConv3(tensorPool2)
        if flow_features is not None:
            tensorConv3 = self.interaction_conv3(tensorConv3 + flow_features[2])
        tensorPool3 = self.modulePool3(tensorConv3)

        tensorConv4 = self.moduleConv4(tensorPool3)
        if flow_features is not None:
            tensorConv4 = self.interaction_conv4(tensorConv4 + flow_features[3])
...
class RGB_Decoder(nn.Module):
    ...
    def forward(self, x, flow_features=None):
        ...
        if flow_features is not None:
            tensorLSTM4 = self.interaction_conv3(tensorLSTM4 + flow_features[3])
        ...
        if flow_features is not None:
            tensorDeconv3 = self.interaction_conv2(tensorDeconv3 + flow_features[2])
        ...
        if flow_features is not None:
            tensorDeconv2 = self.interaction_conv1(tensorDeconv2 + flow_features[1])
...
class RGB_convAE(nn.Module):
    def __init__(self, n_channel=3, t_length=2, latent_dim=128):
        super(RGB_convAE, self).__init__()

        self.encoder = RGB_Encoder(t_length, n_channel, latent_dim)
        self.decoder = RGB_Decoder(t_length, n_channel, latent_dim)

    def forward(self, x, flow_features=None):
        mu, logvar, *encoder_features = self.encoder(x, flow_features)
        output = self.decoder((mu, logvar, *encoder_features), flow_features)
